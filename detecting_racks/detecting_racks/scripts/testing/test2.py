...
# --- Rectangle detection function with pair distance printing ---
...
def find_rectangles_from_points(points, output_img, config):
    ...
    Prints all pairs and their distances for every 4-point combination.
    """
    ...
    for combo in itertools.combinations(points, 4):
        ...
        # --- Print all 6 pairs with distances ---
        pairs = [
            (p1, p2), (p1, p3), (p1, p4),
            (p2, p3), (p2, p4), (p3, p4)
        ]
        ...
        for (a, b) in pairs:
            ...
            d = math.sqrt(d2)
            dists_sq.append(d2)
            print(f"   Pair {a} ↔ {b} → Distance = {d:.2f} px")

        # Sort distances (smallest 4 = sides, largest 2 = diagonals)
        dists_sq.sort()
