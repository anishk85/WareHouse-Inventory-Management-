...
def get_fixed_path_orientation(p_start, p_end):
    """
    Calculates the orientation (quaternion) to face from Start to End.
    This single orientation will be used for ALL goals.
    """
    ...
    if mag > 0:
        ...
        print(f"--- Fixed Orientation Yaw: {math.degrees(yaw):.2f} degrees ---")
...
def main():
    ...
    # This orientation is used for both going forward AND coming back.
    ...
    # Initial Position: Go to Start Point (Facing the intersection)
    ...
    for i in range(ITERATIONS):
        ...
        # We use the SAME q_fixed_z/w. The robot will drive in reverse to get back.
