...
```bash
# Simulation
ros2 launch mecanum_navigation_setup cartographer_mapping.launch.py use_sim:=true
...
```bash
# On hardware
cp ~/my_map.* /workspace/src/mecanum_navigation_setup/maps/

# On simulation PC
cp ~/my_map.* ~/002/src/mecanum_navigation_setup/maps/
```
...
ros2 launch mecanum_navigation_setup cartographer_navigation.launch.py \
  use_sim:=true \
  map:=~/002/src/mecanum_navigation_setup/maps/my_map.yaml
```
...
ros2 launch mecanum_hardware hardware_navigation.launch.py \
  map:=/workspace/src/mecanum_navigation_setup/maps/my_map.yaml
```
...
**Usage**:
```bash
ros2 launch mecanum_navigation_setup cartographer_mapping.launch.py \
  use_sim:=true \
  resolution:=0.05
```
...
**Usage**:
```bash
ros2 launch mecanum_navigation_setup nav2_bringup.launch.py \
  map:=/path/to/my_map.yaml \
...
- [mecanum_gazebo/README.md](../mecanum_gazebo/README.md) - Simulation
