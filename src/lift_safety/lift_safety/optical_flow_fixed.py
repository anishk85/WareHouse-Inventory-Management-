#!/usr/bin/env python3
"""
Optical Flow Safety Node (Fixed)
Detects objects entering lift path using optical flow
Publishes to /lift/stop topic for collision warnings

Run on RPi4 with camera attached
"""
...
class OpticalFlowSafetyNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('stop_topic', '/lift/stop')  # FIXED: Added this parameter
        ...
        self.get_logger().info('OPTICAL FLOW SAFETY NODE')
        self.get_logger().info('=' * 70)
        self.get_logger().info(f"Subscribed to: {video_topic}")
        self.get_logger().info(f"Publishing to: {stop_topic}")
        self.get_logger().info(f"Frame width: {self.FRAME_WIDTH}")
        self.get_logger().info(f"Magnitude threshold: {self.MAGNITUDE_THRESHOLD}")
    ...
    def image_callback(self, msg):
        ...
        # Ignore vertical motion (lift moving up/down)
        ...
        if intrusion_detected:
            self.get_logger().warn('🚨 OBJECT DETECTED IN LIFT PATH!', throttle_duration_sec=1.0)
