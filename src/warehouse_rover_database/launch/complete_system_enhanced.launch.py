#!/usr/bin/env python3
"""
Warehouse Rover Complete Launch File - USB Camera Version
Launches: USB Camera + QR Detector + Inventory Database

Topic Flow:
  USB Camera -> /image_raw -> QR Detector -> /qr_detection -> Inventory Database
                           -> /qr_detection_viz (visualization)
"""
...
def generate_launch_description():
    ...
    # Publishes to: /qr_detection, /qr_detection_viz
    ...
    # ============================================================
    # Database/Inventory Node (MongoDB)
    # ============================================================
    
    # Path to MongoDB parameters YAML file
    mongodb_params_file = PathJoinSubstitution([
        FindPackageShare('warehouse_rover_database'),
        'config',
        'mongodb_params.yaml'
    ])
    
    # Inventory Node with MongoDB Atlas
    # Subscribes to: /qr_detections
    # Stores data in MongoDB Atlas cloud database
    # Configuration loaded from mongodb_params.yaml
    inventory_node = Node(
        package='warehouse_rover_database',
        executable='inventory_node_mongo.py',
        name='inventory_node_mongo',
        parameters=[mongodb_params_file],
        output='screen',
        respawn=True,
        respawn_delay=2.0
    )
    ...
    return LaunchDescription([
        ...
        # Nodes
        # ============================================================
        usb_cam_node,          # USB Camera
        qr_detector_node,      # QR Detection
        inventory_node,        # MongoDB Inventory Management
    ])