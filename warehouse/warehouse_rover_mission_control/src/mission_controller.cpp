  ...
  void MissionController::startOscillation()
{
    RCLCPP_INFO(this->get_logger(), "Starting oscillation...");

    double backdistance = 0.62;
    double frontdistance=0.69;
    double speed = 0.1;
    double frontduration = frontdistance / speed;
    double backduration = backdistance / speed;

    geometry_msgs::msg::Twist cmd;

    auto start = this->now();
    while ((this->now() - start).seconds() < backduration) {
        cmd.linear.x = -speed;
    ...
    start = this->now();
    while ((this->now() - start).seconds() < frontduration) {
        cmd.linear.x = speed;
        cmd_vel_pub_->publish(cmd);
        rclcpp::sleep_for(50ms);
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Oscillation complete. Starting scan dwell...");
  ...
  void MissionController::loadWaypoints()
  {
    ...
    r1.shelf_heights = {0.0, 0.50, 0.75,1.15};
    ...
    r2.shelf_heights = {0.0, 0.50, 0.75,1.15};
    ...
    r3.shelf_heights = {0.0, 0.50, 0.75,1.15};
    ...
    r4.shelf_heights = {0.0, 0.50, 0.75,1.15};
    ...
    r5.shelf_heights = {0.0, 0.50, 0.75,1.15};
...
{
    RCLCPP_INFO(this->get_logger(), "Moving lift to %.2f m", height);
    ...
    double wait_time = 3.0;  // after lift, allow it to settle
    ...
    mission_timer_ = this->create_wall_timer(
        ...
        [this]() {
            ...
            this->processNextShelf(); // ← NOW oscillate, NOT processNextShelf()
        }
    );

    // DO NOT set SCANNING here — scanning happens AFTER oscillation.
}
